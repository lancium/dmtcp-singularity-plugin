...
func callbackPluginCmd(manager *cmdline.CommandManager) {
	...
	var checkpointStartCmd = &cobra.Command{
		...
		Example:               "singularity checkpoint start <image> <name>",
...
func callbackDMTCP(common *config.Common) {
	...
	if isCheckpoint{
		dmtcpLocation := os.Getenv("SINGULARITY_DMTCP_BIN")
		dmtcpLibLocation := os.Getenv("SINGULARITY_DMTCP_LIB")
		if(dmtcpLocation == ""){
			sylog.Errorf("No DMTCP location found. Run install script?")
			return 
		}
		if(dmtcpLibLocation == ""){
			sylog.Errorf("No DMTCP library location found. Run install script?")
			return 
		}
		...
		var hasCkptDir = false
		// Check existing binds for /.checkpoint
		for i := 0; i < len(origBind); i++ {
			if strings.HasPrefix(origBind[i].Destination, "/.checkpoint"){
				hasCkptDir = true
				break;
			}
		}
		if (!hasCkptDir){
			// Create a directory for checkpoints if none specified
			// Uses ./.checkpoint, modify here
			ckptDir := "./.checkpoint"
			...
			//Set to include this new bind paths, no new ckptBind
			c.SetBindPath(append(origBind, dmtcpBind, dmtcpLibBind))
		}
	}
	return
}
